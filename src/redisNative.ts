...
const persistMultiData = (multi: redis.Multi, input: RedisHIncr[]): void => {
    const a: (string | number)[][] = input.map(elem => ["HINCRBY", elem.key, elem.field, elem.incrementValue])
    // client.multi(a).exec()
    input.forEach(elem => multi.hincrby(elem.key, elem.field, elem.incrementValue))
    multi.exec()
}